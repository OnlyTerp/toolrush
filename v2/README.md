...
**Activation state:** **LIVE.** Gateway and desktop backend were cleanly restarted after all sessions went idle on 2026-09-05; live `verify_live_rpc` passed (parallel RPC exercised in the real running kernel, receipt `activation-restart-result.json` verdict DONE). Config and provider settings verified byte-identical (SHA-256) before/after.
