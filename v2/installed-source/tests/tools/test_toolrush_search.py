...
def test_guard_after_window(tmp_path):
    ...
    # Four returned rows plus a sentinel prove this isn't an exhaustive
    # count. The extra row is metadata only, never passed through the guard.
    assert env["total_count"] == 5
    assert env["total_count_is_lower_bound"] is True
    assert len(_norm(env)) == 4
...
def test_context_mode_uses_shared_engine(tree):
    """Context no longer needs a second engine or a special-case bridge."""
    slow = json.loads(ft.search_tool(
        pattern='needle', path=str(tree), context=2, task_id='context-stock',
    ))
    fast = _run('needle', str(tree), context=2, task_id='context-native')
    assert _norm(fast) == _norm(slow)
    assert len(_norm(fast)) > 12
