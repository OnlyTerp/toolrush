...
# ── Read-only terminal classification ──────────────────────────────────────
# Terminal is the most-batched tool in real session data (964 terminal-only
# multi-call batches in 48h, mostly greps/health checks/git status), but it
# sits on no admission list, so every terminal batch runs sequential waves.
# Read-only commands are race-free (no side effects, per-call Popen, cwd is
# snapshotted per call — see LocalEnvironment._run_bash) and can join parallel
# runs. Fail-closed: anything not provably read-only stays a sequential
# barrier exactly as before.

# Commands that only read. `source`/`.` excluded (mutates shell state);
# `cd` IS allowed inside chains (each Popen is isolated; cwd snapshot per call).
_READONLY_TERMINAL_TOKENS = frozenset({
    "cat", "rg", "grep", "ls", "ll", "dir", "find", "head", "tail", "wc",
    "stat", "file", "pwd", "echo", "printf", "which", "type", "whereis",
    "whoami", "hostname", "uname", "date", "env", "printenv", "id", "groups",
    "df", "du", "free", "uptime", "ps", "tasklist", "nproc", "lscpu",
    "diff", "cmp", "md5sum", "sha1sum", "sha256sum", "sha512sum", "cksum",
    "sort", "uniq", "cut", "awk", "column", "nl", "rev", "strings", "sed",
    "tr",  # pure filter — stdin→stdout only, no file args
    "jq", "yq", "bat", "less", "more",
    "readlink", "realpath", "basename", "dirname", "seq", "shuf", "tac",
    "iconv", "xxd", "od", "bc", "expr", "test",
    "git",  # git READ subcommands filtered below; git write verbs rejected
    "curl", "wget", "ping", "ssh-keygen",  # network reads (curl/wget flags checked below)
    "node", "python", "python3",  # scripts CAN write — only allowed with -c/--version probes (see below)
    "npm", "npx",  # only `npm ls`/`npm --version` style probes (filtered below)
    "pip", "uv",   # only list/version probes (filtered below)
    "cd",
    "sha",  # certutil-style hash shorthands on some shells — treat as read
})

# find actionals that mutate — a `find` whose expression contains any of
# these is NOT read-only. Everything else (‑name/-type/-mtime filters,
# -print, -ls, -printf) only reads the tree.
_FIND_MUTATOR_TOKENS = frozenset({
    "-delete", "-exec", "-execdir", "-ok", "-okdir",
    "-fprint", "-fprintf", "-fls",
})

# git subcommands that mutate — everything else git does (status/log/diff/
# show/branch/list/rev-parse/blame/config --get/ls-files/remote) is read-only.
_GIT_WRITE_VERBS = frozenset({
    "add", "commit", "push", "pull", "merge", "rebase", "reset", "clean",
    "checkout", "switch", "restore", "stash", "apply", "cherry-pick",
    "revert", "tag", "init", "clone", "fetch", "mv", "rm", "bisect",
    "worktree", "submodule", "am", "format-patch", "gc", "prune", "filter-branch",
})

# python/node/npm/pip/uv probe shapes that are provably read-only.
_PROBE_READ_ONLY_PATTERNS = re.compile(
    r"""^(?:python3?|py|node|npm|npx|pip3?|uv)\b[^|;&`$]*?
        (?:--version|-v\b|--help|-h\b|\blist\b|\bls\b|\bshow\b|\bwhich\b|\bpath\b)
    """,
    re.VERBOSE,
)

# Overwrite redirect (>file, not >>append, not 2>stderr capture) marks a write.
_REDIRECT_OVERWRITE_RE = re.compile(r"(?:^|[^>2])>{1}(?![>|])\s*\S")


def _is_readonly_terminal_command(command: str) -> bool:
    """True iff a shell command is provably side-effect free. FAIL-CLOSED.

    Whole-command conjunction: every segment of a ``;``/``&&``/``||`` chain
    must independently classify read-only, or the entire command is a write.
    Any unknown/ambiguous construct → False (sequential barrier, today's
    behavior). Bash variable expansion ``$VAR``/``${...}`` and command
    substitution ``$(...)``/backticks → False (can't see through them).
    """
    if not command or not isinstance(command, str):
        return False
    # Explicit read-only syntax is a prerequisite. The legacy permissive
    # token list admitted env arbitrary-code wrappers, git branch writes,
    # wget downloads and shell-state mutations as parallel readers.
    from agent.toolrush_admission import readonly
    if not readonly(command):
        return False
    stripped = command.strip()
    if not stripped:
        return False
    # Command substitution / process substitution: opaque → write-class.
    # Plain $VAR / ${VAR} env expansion is a READ (can't cause side effects on
    # its own) and appears in most real read-only commands ($LOCALAPPDATA/...),
    # so only $(...) and backticks reject the command.
    # Process substitution <( ) is opaque (spawns an arbitrary command) →
    # write-class. Command substitution $() / backticks are classified
    # RECURSIVELY: the inner body must independently prove read-only by the
    # same rules, else the whole command is rejected (one nesting level;
    # deeper nesting stays fail-closed).
    if "<(" in stripped:
        return False
    if "$(" in stripped or "`" in stripped:
        if not _substitutions_are_readonly(stripped):
            return False
        # A substitution riding the args of a network-fetch stage (curl/wget)
        # carries whatever it resolves to (e.g. $(cat token.txt)) straight to
        # the wire — exfiltration vector even with no data flag. Check on the
        # ORIGINAL text: the bodies are blanked below before segmentation.
        for net_stage in re.split(r"\s*(?:;|&&|\|\|)\s*|\n", stripped):
            net_stage = net_stage.strip()
            if not net_stage:
                continue
            head = net_stage.split("|")[0].split()
            if head and head[0].strip("'\"") in {"curl", "wget"}:
                return False
        stripped = re.sub(r"\$\((?:[^()]|\([^()]*\))*\)", " ", stripped)
        stripped = stripped.replace("`", " ")
    # Split on chain separators ; && || and newline. Pipes stay INSIDE a segment
    # (every pipe stage must also be read-only — checked via token scan).
    segments = [s for s in re.split(r"\s*(?:;|&&|\|\|)\s*|\n", stripped) if s.strip()]
    if not segments:
        return False
    for segment in segments:
        if not _segment_is_readonly(segment):
            return False
    return True


def _segment_is_readonly(segment: str) -> bool:
    """Classify one chain segment (pipes allowed; every stage must be read-only)."""
    if _REDIRECT_OVERWRITE_RE.search(segment):
        return False
    # `>>` append is still a file WRITE — reject the whole segment.
    if ">>" in segment:
        return False
    # `tee` writes; `touch` writes; `xargs` can run anything — reject the segment.
    if re.search(r"\b(?:tee|touch|xargs|shred|dd|install|cp|mv|rm|rmdir|mkdir|ln|chmod|chown|kill|pkill|taskkill|truncate|tee)\b", segment):
        return False
    # sudo runs anything as root — reject.
    if re.search(r"\bsudo\b", segment):
        return False
    # in-place editor flags: sed -i, gawk --inplace, etc.
    if re.search(r"\bsed\b[^|]*\s-i\b", segment):
        return False
    # python/node/npm/pip/uv: only explicit probe shapes pass.
    if re.match(r"^\s*(?:python3?|py|node|npm|npx|pip3?|uv)\b", segment):
        if not _PROBE_READ_ONLY_PATTERNS.match(segment):
            return False
    # stages split on pipes — every stage's first token must be a read token
    ...
    for stage in stages:
        ...
        # strip leading env-var assignments (FOO=bar cmd) — classify the cmd
        while "=" in tokens[0] and not tokens[0].startswith("-") and re.match(r"^\w+=", tokens[0]):
            tokens = tokens[1:]
            if not tokens:
                return True  # pure assignments, no command — no side effects
        if not tokens:
            return True
        first = tokens[0].strip("'\"")
        if first not in _READONLY_TERMINAL_TOKENS:
            return False
        if first == "git":
            # find the git subcommand (skip global flags like -C path)
            rest = tokens[1:]
            ci = 0
            while ci < len(rest) and rest[ci].startswith("-"):
                # -C takes a path argument
                if rest[ci] in {"-C", "--git-dir", "--work-tree"} and ci + 1 < len(rest):
                    ci += 2
                else:
                    ci += 1
            sub = rest[ci] if ci < len(rest) else ""
            if sub in _GIT_WRITE_VERBS:
                return False
        elif first == "find":
            if any(tok in _FIND_MUTATOR_TOKENS for tok in tokens[1:]):
                return False
        elif first in {"curl", "wget"}:
            # Network reads only. Two exfiltration vectors are rejected:
            # 1. Any data-carrying / upload / output flag.
            # 2. Any command substitution in the stage — the substituted
            #    value (e.g. $(cat token.txt)) rides the URL/args to the
            #    network even with no explicit -d flag.
            _stage_text = " ".join(tokens[1:])
            if "$(" in _stage_text or "`" in _stage_text:
                return False
            _net_write = re.compile(
                r"(?:^|\s)(?:-{1,2}(?:d\b|data|data-binary|data-raw|data-urlencode|"
                r"T\b|upload-file|F\b|form|form-string|form-escape|"
                r"O\b|remote-name|output|out|\bJ\b|remote-header-name)|"
                r"--post\d+|--put)",
            )
            if _net_write.search(" " + _stage_text + " "):
                return False
    return True


def _substitutions_are_readonly(command: str) -> bool:
    """Classify the INNER bodies of $() / backtick substitutions (1 level).

    Returns True iff every substitution body classifies read-only by the
    same segment rules. Any unbalanced paren, nested substitution, or
    unclassifiable inner command → False (fail-closed).
    """
    bodies: list[str] = []
    i = 0
    n = len(command)
        ...
        if command.startswith("$(", i):
            ...
            while j < n and depth:
                ...
                if cj == "\\":
                    j += 2
                    continue
                if command.startswith("$(", j) or cj == "`":
                    return False  # nested substitution — fail closed
                if cj == "(":
                    depth += 1
                elif cj == ")":
                    depth -= 1
    ...
    for body in bodies:
        if not body.strip():
            return False
        for seg in [s for s in re.split(r"\s*(?:;|&&|\|\|)\s*|\n", body) if s.strip()]:
...
def _plan_tool_batch_segments(tool_calls, *, execution_cwd: Optional[Path] = None) -> List[tuple]:
    ...
    * Anything not in ``_PARALLEL_SAFE_TOOLS`` and not an opted-in MCP
    ...
    for tool_call in tool_calls:
        ...
        # Read-only terminal admission: provably side-effect-free shell
        # commands (greps, health checks, git status, ls — the most-batched
        # shapes in real session data) join the parallel run. Writes and
        # anything unclassifiable stay sequential barriers. A read-only
        # terminal reserves the WHOLE cwd subtree as a READER (shell cwd
        # state is per-call but filesystem visibility spans the cwd), so
        # any write_file/patch/write-terminal against an overlapping path
        # still orders correctly behind/ahead of these.
        if effective_name == "terminal":
            terminal_command = ""
            if isinstance(effective_args, dict):
                _cmd = effective_args.get("command")
                if isinstance(_cmd, str):
                    terminal_command = _cmd
            if _is_readonly_terminal_command(terminal_command):
                _term_cwd = execution_cwd if execution_cwd is not None else Path.cwd()
                _term_scope = [_canonical_path(str(getattr(effective_args, "get", lambda *_: None)("workdir") or "."), execution_cwd=_term_cwd)]
                if any(
                    existing_is_writer and _paths_overlap(p, existing)
                    for p in _term_scope
                    for existing, existing_is_writer in reserved_paths
                ):
                    _close_parallel()
                reserved_paths.extend((p, False) for p in _term_scope)
                current.append(tool_call)
                continue
            _add_sequential(tool_call)
            continue
