...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    Debounced: the per-call fsync-class SQLite write is pure overhead for
    read-only tools, so non-forced flushes are deferred to the segment/turn
    boundary (``flush_pending_session_db``) unless ``force`` is set. Callers
    MUST pass ``force=True`` after write-class tool results and before any
    authorization/approval wait — those are the crash-safety points the
    debounce exists to protect.
    """
    if not force:
        agent._pending_session_db_flush = True
        return True
    agent._pending_session_db_flush = False
    try:
...
def _result_is_write_class(agent, function_name: str, function_args: dict) -> bool:
    """True when this call's RESULT could have side effects that outlive the
    process (destructive shell, file writes/patches, MCP tools) — the cases
    where the transcript flush must stay synchronous per call."""
    if function_name in {"terminal", "write_file", "patch", "code_execution",
                         "execute_code", "delegate"}:
        return True
    # MCP tools run external servers — treat as write-class (fail-safe).
    return function_name.startswith("mcp__")

def flush_pending_session_db(agent, messages: list, *, stage: str) -> bool:
    """Drain a debounced flush at a segment/turn/pre-approval boundary.

    Returns False when persistence is broken (mirrors
    _flush_session_db_after_tool_progress). No-op when nothing is pending.
    """
    if not getattr(agent, "_pending_session_db_flush", False):
        return True
    return _flush_session_db_after_tool_progress(agent, messages, stage=stage, force=True)



# Session-keyed registry of live (agent, messages) pairs, populated when an
# executor path starts a tool batch and drained by approval.py's
# human_wait_window BEFORE the human wait parks the turn. Keyed by session
# (not a module-global pointer) because the multiplexed gateway runs many
# sessions' tool batches concurrently in this process; approval waits are
# already keyed the same way, so the drain hits exactly the right transcript.
_PENDING_FLUSH_BY_SESSION: dict[str, tuple[Any, list]] = {}
_PENDING_FLUSH_REGISTRY_LOCK = threading.Lock()


def register_pending_flush(session_key: str, agent: Any, messages: list) -> None:
    """Register (agent, messages) so a pre-approval drain can flush them."""
    with _PENDING_FLUSH_REGISTRY_LOCK:
        _PENDING_FLUSH_BY_SESSION[session_key] = (agent, messages)


def pop_pending_flush_registration(session_key: str) -> tuple[Any, list] | None:
    """Remove and return the session's registration, or None."""
    with _PENDING_FLUSH_REGISTRY_LOCK:
        return _PENDING_FLUSH_BY_SESSION.pop(session_key, None)
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        ...
        # covers every tool.
        #
        # Fast-path skip: the heartbeat only matters for calls that can outlive
        # the watchdog's idle window. Expected-fast tools (file reads/writes,
        # searches, in-memory registries) finish in milliseconds — starting and
        # joining a thread per call was pure overhead. Anything that can block
        # on the outside world (terminal, network, browser, approval/clarify,
        # unknown tools) keeps the heartbeat. The in-flight tool stamps
        # activity at start AND completion regardless, so a "fast" tool that
        # unexpectedly stalls is still covered by the batch/sequential
        # deadline and the concurrent path's own 30s heartbeat.
        _FAST_HEARTBEAT_TOOLS = frozenset({
            "read_file", "write_file", "patch", "search_files", "session_search",
            "skill_view", "skills_list", "tool_search", "tool_describe", "todo_list",
        })
        _needs_heartbeat = (
            function_name not in _FAST_HEARTBEAT_TOOLS
            or authorization_gate is not None
        )
...
# Shared worker pool for the sequential tool path. One pool for the process
# instead of a fresh DaemonThreadPoolExecutor(max_workers=1) per call: each
# call submits a single future, so per-call isolation is preserved while
# thread/executor construction cost drops to zero. The pool holds daemon
# threads and is never joined at exit (daemon=True), matching the old
# per-call executors' atexit behavior.
_SEQUENTIAL_EXECUTOR: DaemonThreadPoolExecutor | None = None
_SEQUENTIAL_EXECUTOR_LOCK = threading.Lock()


def _get_sequential_executor() -> DaemonThreadPoolExecutor:
    """Return the process-wide sequential-path worker pool (lazy-created)."""
    global _SEQUENTIAL_EXECUTOR
    with _SEQUENTIAL_EXECUTOR_LOCK:
        if _SEQUENTIAL_EXECUTOR is None:
            from tools.daemon_pool import DaemonThreadPoolExecutor as _DPE
            _SEQUENTIAL_EXECUTOR = _DPE(max_workers=1)
...
def _run_sequential_tool_execution_middleware(
    ...
    *,
    ...
    # Shared process-wide pool — one future per call. Per-call isolation of
    # RESULTS is preserved, but a WEDGED worker occupies the pool's single
    # slot forever, so the finally block retires a pool whose worker didn't
    # come back (see below) — the old per-call executor threw the wedged
    # thread away with its executor; retirement reproduces exactly that.
    executor = _get_sequential_executor()
    ...
    try:
        ...
        # Healthy path: the future is done (returned above) and the worker is
        # free for the next call — nothing to do.
        #
        # Abandon path (interrupt grace expired / deadline): the worker may
        # still be running the tool. A wedged worker occupies this pool's
        # ONLY slot (max_workers=1): a later submit would queue behind it
        # forever and every subsequent sequential call process-wide would
        # spin to its own deadline without ever starting. Retire the pool —
        # swap the global to a fresh executor — when the global still points
        # at the pool this future ran on (another thread's retirement must
        # not clobber a newer pool). The retired executor is shut down with
        # wait=False: its wedged worker stays a detached daemon thread,
        # matching the old per-call abandon-shutdown semantics exactly.
        # In-flight futures other agents submitted to the SAME retired pool
        # keep running; their own abandon paths retire again in turn.
        if timed_out and not future.done():
            global _SEQUENTIAL_EXECUTOR
            with _SEQUENTIAL_EXECUTOR_LOCK:
                if _SEQUENTIAL_EXECUTOR is executor:
                    _SEQUENTIAL_EXECUTOR = None
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    # Expose (agent, messages) to approval.py's human_wait_window so a
    # debounced flush can be forced before the turn parks on a human prompt.
    try:
        from tools.approval import get_current_session_key
        register_pending_flush(get_current_session_key(), agent, messages)
    except Exception:
        pass
    ...
    for i, (tc, name, args, middleware_trace, _parse_error, _scope_block) in enumerate(
        ...
        if not _flush_session_db_after_tool_progress(
            ...
            force=_result_is_write_class(agent, name, args),
        ):
            return
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if not _flush_session_db_after_tool_progress(
            ...
            force=_result_is_write_class(agent, function_name, function_args),
        ):
            return
    ...
    # Turn-end drain for the standalone sequential path (when this executor is
    # the whole batch, not a segment of execute_tool_calls_segmented). Segmented
    # runs drain at each segment boundary instead.
    if finalize and not flush_pending_session_db(
        agent, messages, stage="sequential turn end"
    ):
        return
...
def execute_tool_calls_segmented(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, segments=None) -> None:
    ...
    *k+1..n* without executing them while preserving one result per
    ...
    for kind, calls in segments:
        ...
        # Segment boundary: drain any debounced per-result flushes before the
        # next segment's tools run. Write-class results already flushed inline.
        if not flush_pending_session_db(
            agent, messages, stage=f"segment boundary ({kind} x{len(calls)})"
        ):
            return
        if getattr(agent, "_incremental_persistence_failed", False):
            return
