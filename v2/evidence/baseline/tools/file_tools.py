...
def _toolrush_fast_search_enabled() -> bool:
    """Kill-switch for the ToolRush in-process search lane.

    Env TOOLRUSH_SEARCH=0 forces OFF (negative controls); config
    toolrush.fast_search=false forces OFF; default ON. Only meaningful on
    the local backend — remote/docker backends never enable the lane
    (checked in _toolrush_fast_search).
    """
    import os as _os
    if _os.environ.get("TOOLRUSH_SEARCH", "1") != "1":
        return False
    try:
        from hermes_cli.config import load_config, cfg_get
        cfg = load_config()
        return bool(cfg_get(cfg, "toolrush", "fast_search", default=True))
    except Exception:
        return True


def _toolrush_fast_search(pattern: str, path: str, file_glob, limit: int,
                          offset: int, output_mode: str, context: int,
                          task_id: str) -> Optional[dict]:
    """Bridge search_tool -> the ToolRush in-process engine, or None to
    fall back to the rg lane. Local-backend gate mirrors the fast-read
    lane's (_file_ops_uses_host_paths)."""
    try:
        ops = _get_file_ops(task_id)
        if not _file_ops_uses_host_paths(ops):
            return None
        # context>0 and multiline patterns are rg-lane features; the
        # in-process engine is line-scoped and must refuse them.
        if (context or 0) > 0:
            return None
        # Path-shape parity: the rg lane passes the root through
        # _expand_path + _escape_native_tool_arg, so rg ECHOES an
        # all-forward-slash root (`C:/x/tree\d1\a.py`). The engine joins
        # root_input verbatim, so give it the same normalized form.
        try:
            root_input = ops._expand_path(path)
            from tools.environments.local import _IS_WINDOWS, _msys_to_windows_path
            if _IS_WINDOWS and root_input:
                root_input = _msys_to_windows_path(root_input).replace("\\", "/")
        except Exception:
            root_input = path
        ...
        return fast_search(
            ...
            resolved_root=str(_resolve_path_for_task(path, task_id)),
            file_glob=file_glob,
            limit=limit,
            offset=offset,
            output_mode=output_mode,
            task_id=task_id,
            guard_fn=_search_result_read_block_error,
            redact_fn=lambda text: redact_sensitive_text(text, file_read=True),
            regex_module=re,
        )
    except Exception:
        return None
...
def search_tool(pattern: str, target: str = "content", path: str = ".",
    ...
    try:
        ...
        # ── ToolRush fast lane (in-process search) ────────────────────
        # Local backend only: pure-Python walk replaces the rg spawn +
        # per-match guard walk. Guards below (root block check above,
        # per-file read-block guard inside the lane, redaction) all still
        # run. Kill-switch: config toolrush.fast_search=false or env
        # TOOLRUSH_SEARCH=0 (env wins for negative controls). Any case the
        # lane can't prove equivalent to rg (context mode, multiline,
        # missing root, binary edge) fails OPEN to the rg path below.
        if _toolrush_fast_search_enabled() and target == "content":
            try:
                fast = _toolrush_fast_search(
