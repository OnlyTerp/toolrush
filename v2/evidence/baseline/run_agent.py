...
class AIAgent:
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        """
        # claude-shim (2026-09-01): the local shim translates OpenAI-style
        # reasoning={"enabled":true,"effort":X} into Anthropic's top-level
        # output_config.effort for adaptive-thinking models (Fable 5.1 et al),
        # so /reasoning levels must reach it. Verified live: effort=high/medium
        # requests 200 with thinking=adaptive. Port-precise so other local
        # shims (codex 18777, antigravity 18778, ...) are untouched.
        try:
            from urllib.parse import urlparse

            _shim_port = urlparse(self._base_url_lower).port
        except Exception:
            _shim_port = None
        if _shim_port == 18776:
            return True
